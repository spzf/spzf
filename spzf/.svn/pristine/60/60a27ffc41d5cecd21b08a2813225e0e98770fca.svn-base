...
<script type="text/javascript">
	...
	$('#systemMenu').tree({
		...
		onSelect: function(node){
			$.post('${basePath}managment/module/getAll.do?sessionId=${sessionId}',{systemId : node.id},function(data){
	...
	function add(pid) {
		...
		$('#addUI').dialog({    
		    ...
		    closed: false,    
		    cache: false,    
		    href: "${basePath}managment/module/insertUI.do?sessionId=${sessionId}&pid="+pid+"&systemId="+node.id,    
		    modal: true,
		    buttons : [{
				text:'保存',
				handler:function(){
					...
					if(form.form('validate')){
						...
						$.post(
							'${basePath }managment/module/insert.do?sessionId=${sessionId}',
							data,
							function(result){
	...
	$.extend($.fn.validatebox.defaults.rules, {
	    ...
	    validator: function(value, param){
	    	...
	    	$.ajax({
	    		async:false,
	    		url:'${basePath}managment/module/checkSnExsits.do?sessionId=${sessionId}',
	...
	function edit() {
		...
		$('#editUI').dialog({    
		    ...
		    closed: false,    
		    cache: false,    
		    href: "${basePath}managment/module/updateUI.do?sessionId=${sessionId}",
		    modal: true,
		    buttons : [{
				text:'保存',
				handler:function(){
					...
					if(form.form('validate')){
						...
						$.post(
							'${basePath }managment/module/update.do?sessionId=${sessionId}',
							fdata,
							function(result){
								$.messager.progress('close');
								if(result.responseCode==100){
		...
		if (rows && rows.length > 0) {
			...
			$.messager.confirm("信息", "确定删除选中记录？", function(r) {
				if (r) {
					...
					var id = ids.join(',');
					$.ajax({
						url : "${basePath}managment/module/delete.do?sessionId=${sessionId}&ids=" + id,
						dataType : 'json',
	...
	function deletePriVal(imgobj){
		$.ajax({
			type:'post',
			url : "${basePath}managment/module/deletePriVal.do",
	...
	function addfunction() {
		...
		if(rows && rows.length==1){
			...
			$('#addPValUI').dialog({    
			    title: '分配权限值:'+rows[0].name,    
			    width: 500,    
			    height: 200,    
			    closed: false,    
			    cache: false,    
			    href: "${basePath}managment/module/insertPriValUI.do?sessionId=${sessionId}&systemId="+node.id+"&moduleId="+rows[0].id+"&moduleSn="+rows[0].sn,  
			    modal: true,
			    buttons : [{
					text:'保存',
					handler:function(){
						...
						$.post(
							'${basePath }managment/module/insertPriVal.do?sessionId=${sessionId}&moduleId='+rows[0].id,
							data,
							function(result){
								$.messager.progress('close');
								if(result.responseCode==100){
					...
					text:'关闭',
					handler:function(){$('#addPValUI').dialog('close');$('#addPValUI').remove();}
				}]
			});    
		}else{
			$.messager.alert('信息','请选择一条记录！','info');
