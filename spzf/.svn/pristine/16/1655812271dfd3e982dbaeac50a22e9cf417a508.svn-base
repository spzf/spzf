...
<script type="text/javascript">
	...
	$('#systemMenu').tree({
		...
		onSelect: function(node){
			$.post("${basePath}managment/dictionary/getAll.do?sessionId=${sessionId}",{systemSn : node.sn},function(data){
	...
	function add(pcode) {
		...
		$('#addUI').dialog({    
		    ...
		    closed: false,    
		    cache: false,    
		    href: "${basePath}managment/dictionary/insertUI.do?sessionId=${sessionId}&pcode="+pcode+"&systemSn="+node.sn,    
		    modal: true,
		    buttons : [{
				text:'保存',
				handler:function(){
					...
					if(form.form('validate')){
						...
						$.post(
							'${basePath}managment/dictionary/insert.do?sessionId=${sessionId}',
							data,
							function(result){
	...
	$.extend($.fn.validatebox.defaults.rules, {
	    ...
	    validator: function(value, param){
	    	...
	    	$.ajax({
	    		async:false,
	    		url:'${basePath}managment/dictionary/checkDictionaryCodeExsits.do?sessionId=${sessionId}',
	...
	function edit() {
		...
		$('#editUI').dialog({    
		    ...
		    closed: false,    
		    cache: false,    
		    href: "${basePath}managment/dictionary/updateUI.do?sessionId=${sessionId}",
		    modal: true,
		    buttons : [{
				text:'保存',
				handler:function(){
					...
					if(form.form('validate')){
						...
						$.post(
							'${basePath }managment/dictionary/update.do?sessionId=${sessionId}',
							fdata,
							function(result){
								$.messager.progress('close');
								if(result.responseCode==100){
		...
		if (rows && rows.length > 0) {
			...
			$.messager.confirm("信息", "确定删除选中记录？", function(r) {
				if (r) {
					...
					var id = ids.join(',');
					$.ajax({
						url : "${basePath}managment/dictionary/delete.do?sessionId=${sessionId}&ids=" + id,
						dataType : 'json',
