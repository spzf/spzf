...
<body class="easyui-layout">
	<div class="header" data-options="region:'north',split:true,border:true" style="height:105px;">
		<div class="header_nav_index">
            ...
            <div class="bd" style="text-align:right;">
            	<span>欢迎您,${login_user.realName }</span>
    ...
    <div data-options="region:'south',split:true,border:false" style="height:36px;text-align:center;line-height:36px;">
    	SPZF后台管理系统&copy; 2016
    </div>
    <div id="leftMenuPanel" data-options="region:'west',title:'权限系统菜单',split:true,border:true" style="width:150px;">
    	<ul id="leftMenu" class="easyui-tree" url="${basePath }managment/frame/systemTree.do?systemId=${systemId}&url=${systemUrl}&sessionId=${sessionId}" data-options="loadFilter : pagerFilter" style="width:100%;height:100%;">
        </ul>
    </div>
    <div data-options="region:'center',border:true" style="background:#eee;">
    	<div id="mainTabs" class="easyui-tabs" data-options="border:false,fit:true,tools:'#tabsButtons'" >
            <div name="first" title="系统桌面" iconCls="icon-home" fit="true">
            	<iframe id="deskFrame" style="border:none;width:100%;height:100%;" src="${basePath }managment/frame/index.do?sessionId=${sessionId}" frameborder="0" scrolling="none"></iframe>
            </div>
        </div>
    </div>
...
function resetpassword(){
	...
	$('#resetpwd').dialog({    
	    ...
	    closed: false,    
	    cache: false,    
	    href: '${basePath}managment/user/rePasswordUI.do?sessionId=${sessionId}',    
	    modal: true,
	    buttons : [{
			text:'保存',
			handler:function(){
				...
				if(form.form('validate')){
					...
					$.post(
						'${basePath }managment/user/rePassword.do?sessionId=${sessionId}',
						data,
						function(result){
							...
							if(result.responseCode==100){
								...
								$.messager.alert('信息','保存失败:'+text.responseMsg,'info');
							}
						},'json'
					);
				}
