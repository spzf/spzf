...
function add(dialog){
	...
	if(form.form('validate')){
		...
		$.post(
			basePATH + 'managment/icsystem/insert.do?sessionId='+sessionId,
			data,
			function(result){
...
function edit(dialog){
	...
	if(form.form('validate')){
		var data=form.form("getData");
		$.post(
			basePATH + 'managment/icsystem/update.do?sessionId='+sessionId,
			data,
			function(result){
				if(result.responseCode==100){
					dialog.close();
...
$.extend($.fn.validatebox.defaults.rules, {
	unique: {
		validator: function(value, param){
			...
			$.ajax({
				async:false,
				url:basePATH + 'managment/icsystem/checkSnExsits.do?sessionId='+sessionId,
