...
function getTabByTitle(title){
	...
	for(var i=0,j=tabs.length;i<j;i++){
		if($.trim(tabs[i].title)==$.trim(title)){
			currentTab=Index.tabs.getTab(tabs[i].name);
			return currentTab;
		}
	}
	return currentTab;
...
function addBlankTab(node) {
	var tab=Index.tabs.tabs('getTab',node.text+"<span style='display:none;'>"+node.id+"</span>");
	if(!tab){
		Index.tabs.tabs('add',{
			title: node.text+"<span style='display:none;'>"+node.id+"</span>",
			fit: true,
			closable:true,
			content:'<iframe style="border:none;width:100%;height:100%;" src="'+node.url+'" frameborder="0" scrolling="none"></iframe>'
		});
